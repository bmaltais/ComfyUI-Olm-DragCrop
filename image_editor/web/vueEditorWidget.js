...
// Helper function to update image preview size based on available node spacefunction updateImagePreviewSize(node, imagePreview) {  if (!imagePreview || !imagePreview.naturalWidth) return;  const nodeWidth = node.size?.[0] ?? 300;  const nodeHeight = node.size?.[1] ?? 140;  // Calculate available space for image (subtract button and padding space)  const buttonHeight = 36; // Edit button height + margin  const padding = 32; // Top/bottom padding  const availableImageHeight = nodeHeight - buttonHeight - padding;  const availableImageWidth = nodeWidth - padding;  // Ensure minimum viable space but allow much larger images  const effectiveImageHeight = Math.max(80, availableImageHeight);  const effectiveImageWidth = Math.max(100, availableImageWidth);  // Reset any previous constraints  imagePreview.style.minHeight = "";  imagePreview.style.minWidth = "";  // Update image styling to fill available space more aggressively  imagePreview.style.maxHeight = `${effectiveImageHeight}px`;  imagePreview.style.maxWidth = `${effectiveImageWidth}px`;  imagePreview.style.width = "auto";  imagePreview.style.height = "auto";  imagePreview.style.objectFit = "contain";  // Force the image to scale up by using min-height/min-width when there is lots of space  if (effectiveImageHeight > 150) {    imagePreview.style.minHeight = `${Math.min(effectiveImageHeight * 0.7, 300)}px`;  }  if (effectiveImageWidth > 200) {    imagePreview.style.minWidth = `${Math.min(effectiveImageWidth * 0.7, 400)}px`;  }  console.log("[ImageEditor] Image preview resized to fill available space:", {    nodeSize: `${nodeWidth}x${nodeHeight}`,    availableSpace: `${effectiveImageWidth}x${effectiveImageHeight}`,    constraints: {      maxHeight: `${effectiveImageHeight}px`,      maxWidth: `${effectiveImageWidth}px`,      minHeight: imagePreview.style.minHeight,      minWidth: imagePreview.style.minWidth    },    imageNaturalSize: `${imagePreview.naturalWidth}x${imagePreview.naturalHeight}`  });}
...
export async function createImagePreviewWidget(node) {
  ...
  const widget = node.addDOMWidget('image_preview', 'div', container, {
    ...
    hideOnZoom: false,
    computeSize() {
      return [node.size?.[0] ?? 300, node.size?.[1] ?? 140];
    },
    onResize() {
      // When node is resized, update image preview to fill available space
      const widgetData = previewWidgets.get(node.id);
      if (widgetData) {
        const nodeWidth = node.size?.[0] ?? 300;
          const nodeHeight = node.size?.[1] ?? 140;
          
          // Always update container to fill available space
  ...
  // Set up ResizeObserver to watch for node size changes and update image sizing
  if (window.ResizeObserver) {
    const resizeObserver = new ResizeObserver(() => {
      const widgetData = previewWidgets.get(node.id);
      if (widgetData) {
        // Delay slightly to ensure node.size is updated
        setTimeout(() => {          console.log("[ImageEditor] ResizeObserver or onResize triggered - updating sizing...");          
          const nodeWidth = node.size?.[0] ?? 300;
          const nodeHeight = node.size?.[1] ?? 140;
...
// Add additional periodic check for missed resize events    const periodicCheck = setInterval(() => {      const widgetData = previewWidgets.get(node.id);      if (widgetData && widgetData.imagePreview && widgetData.imagePreview.style.display !== "none") {        updateImagePreviewSize(node, widgetData.imagePreview);      }    }, 500);    previewWidgets.get(node.id).periodicCheck = periodicCheck;
  }
...
}    // Clean up periodic check if it exists    if (widgetData.periodicCheck) {      clearInterval(widgetData.periodicCheck);
